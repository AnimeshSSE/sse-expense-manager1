...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build the parameterized SQL query safely using Prisma.sql tagged template
    // Prisma stores DateTime as BigInt (Unix ms). Use unixepoch modifier.
    const startMs = startDate.getTime();
    const sqlConditions: Prisma.Sql[] = [
      Prisma.sql`expenseDate >= ${startMs}`,
      Prisma.sql`status != 'REJECTED'`,
    ];
    if (siteId) sqlConditions.push(Prisma.sql`siteId = ${siteId}`);
    if (userId) sqlConditions.push(Prisma.sql`userId = ${userId}`);
    if (clientId) sqlConditions.push(Prisma.sql`siteId IN (SELECT id FROM "Site" WHERE clientId = ${clientId})`);
