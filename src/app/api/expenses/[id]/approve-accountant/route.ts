...
// POST /api/expenses/[id]/approve-accountant
export async function POST(
  ...
  try {
    ...
    if (expense.status !== ExpenseStatus.PENDING) {
      return NextResponse.json({ error: `Cannot approve: expense status is ${expense.status}, expected PENDING` }, { status: 400 })
    }
    ...
    console.error('POST /api/expenses/[id]/approve-accountant error:', error)
