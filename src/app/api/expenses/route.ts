...
export async function GET(request: NextRequest) {
  try {
    ...
    if (userRole === "EMPLOYEE" && userId) {
      where.userId = userId;
    } else if (userRole === "STOCK_MANAGER" && userId) {
      // Stock Manager sees all requisitions but only own expenses
      where.userId = userId;
    }
    // MANAGER and ADMIN see all records

    if (status) {
      where.status = status;
    }

    if (userId && (userRole === "MANAGER" || userRole === "ADMIN" || !userRole)) {
      // Admin/Manager can optionally filter by specific user
      // Don't override role-based filter
    } else if (userId && userRole !== "EMPLOYEE" && userRole !== "STOCK_MANAGER") {
      where.userId = userId;
    }
