...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < dataLines.length; i++) {
      ...
      const [name, code, address, city, state, pincode] = columns;
      ...
      if (existingName) {
        ...
        results.errors.push({
          row: i + 2,
          message: `Site with name '${name}' already exists`,
        });
        continue;
      }
    ...
    return NextResponse.json(
      {
        message: `Upload complete: ${results.success} sites created, ${results.failed} failed`,
        ...results,
      },
      { status: 201 }
    );
  } catch (error) {
    console.error("Sites bulk upload error:", error);
    return NextResponse.json(
