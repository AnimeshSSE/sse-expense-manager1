...
import { Plus, Pencil, Trash2, Loader2, Tags, Upload, Download, FileSpreadsheet, X, CheckCircle2, AlertTriangle, AlertCircle } from 'lucide-react'
...
export function CategoriesPage() {
  ...
  const expectedHeaders = ['Name', 'Type', 'Description', 'IsActive']
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/categories/bulk-upload', { method: 'POST', body: formData })
      const data = await res.json()
      if (!res.ok) { toast({ title: 'Upload failed', description: data?.errors?.[0]?.error || 'Unknown error', variant: 'destructive' }); return }
      setBulkResult(data)
      if (data.created > 0) {
        toast({ title: 'Upload Complete', description: `${data.created} categories created${data.errors.length > 0 ? ` with ${data.errors.length} errors` : ''}` })
        loadCategories()
      }
  ...
  const handleBulkDownloadTemplate = async () => {
    try {
      const res = await fetch('/api/categories/bulk-upload/template')
      if (!res.ok) throw new Error()
      const blob = await res.blob(); const url = URL.createObjectURL(blob)
      const a = document.createElement('a'); a.href = url; a.download = 'category-bulk-upload-template.xlsx'
      ...
      <Dialog open={bulkOpen} onOpenChange={handleBulkClose} modal>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Upload an xlsx, xls, or csv file with category data. Max 500 rows.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
              ...
              <div className="space-y-3">
                {bulkResult.created > 0 && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-3 flex items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-green-800">{bulkResult.created} categories created successfully</p>
                  </div>
                )}
